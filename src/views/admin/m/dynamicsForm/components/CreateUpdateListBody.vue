<template>
    <div v-if="pageDataLoaded">
        <div class="search-area">
            <template v-if="pageData.statisticGroups&&pageData.statisticGroups.length">
                <el-row :gutter="20" type="flex" style="margin-bottom: 15px;flex-wrap: wrap;">
                    <el-col
                        v-for="(el, index) in pageData.statisticGroups"
                        :key="index"
                        :style="{width: pageData.statisticGroups.length>6?'16.66667%':undefined}"
            ...
            >
                <el-form-item v-for="(el, index) in queryTagsRender" :key="index" :label="el.groupName+'：'" style="margin-left: 10px;">
                    <el-radio-group v-model="tagGroups[queryTagsGroupKey + index]" @change="handleQueryTagsChange($event,
                    queryTagsGroupKey, index)">
        ...
        <div class="page-content">
            <div class="page-content-body">
                <TableWrapper
                    ...
                    @onSuccessData="onTableDataSuccess"
                >
                    <template slot="tool">
                        <el-button v-for="(el, index) in actionBtns" :key="index" type="primary" plain @click="el.click">
                            <i
                                v-if="el.icon"
                                :class="el.icon"
                                class="iconfont "></i>
...
export default {
    name: 'CreateUpdateListBody',
    components: { TableWrapper, Mallki },
    props: {
        pageData: Object, // 页面数据
        pid: String, // 父表单的模型id
        rowId: String // 父表单的详情记录id
    },
    data () {
        return {
            formId: null, // 页面的表单id
    ...
    methods: {
        ...
        initPage (listJson) {
            ...
            this.listQuery[listJson.foreignKey] = this.rowId || -1
            this.listQuery.id = this.pid
            this.listQuery.isTab = true
            this.listQuery.subFormId = listJson.formId
            ...
            if (listJson.topButtons) {
                this.actionBtns = listJson.topButtons.map(item => {
                    return {
                        text: item.name,
        ...
        // 表格获取完数据的回调
        onTableDataSuccess (tableData) {
            const columnsKeys = {}
            this.pageData.queryColumns.forEach(item => {
        ...
        getAttachment (nameKey, row) {
            const { id } = row
            Api_getAttachments(nameKey, id).then(res => {
                if (res && res.length) {
                    this.$set(row, nameKey, res.map(item => ({
                        url: item.url,
