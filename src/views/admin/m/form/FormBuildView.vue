    ...
    <div>
        ...
        >
            <el-row>
                ...
                <el-col
                    :md="leftFormOpened ? 16 : 22"
                    :lg="leftFormOpened ? 18 : 23"
                    ...
                    >
                        <div class="item-area" style="margin-bottom: 15px;">
                            <el-form-item label="主键" prop="primaryKey" class="fl">
                                ...
                                <el-input
                                    v-model="addFormData.primaryKey"
                                    style="width: auto"
                                />
                            </el-form-item>
                            <el-form-item label="外键" prop="foreignKey" class="fl">
                                <template slot="label">
                                    ...
                                    <el-tooltip
                                        class="item"
                                        effect="dark"
                                        content="格式：({table}){column},如果该模型不作为子模型，则可不填；如果作为子模型则填入外键，“table”必需与主键的“table”保持一致"
                                    >
                                        <i class="iconfont icon-question-circle"></i>
                                    </el-tooltip>
                                </template>
                                <el-input
                                    v-model="addFormData.foreignKey"
                                    style="width: auto"
                                />
                            </el-form-item>
                            <el-form-item
                            ...
                            <el-form-item label="每行记录数" prop="rowColumns" class="fl">
                                <template slot="label">
                                    每行记录数
                                    <el-tooltip
                                        class="item"
                                ...
                                <el-input
                                    v-model="addFormData.rowColumns"
                                    style="width: auto"
                                />
                            </el-form-item>
                        </div>
                        <el-form-item class="item-area" label="额外操作按钮">
                            <template slot="label">
                                额外操作按钮
                                <!--                                <i-->
                                <!--                                    class="iconfont icon-image icon-image-extend"-->
                            ...
                            >
                                <el-tag
                                    v-for="(tag, index) in buttons"
                                    :key="index"
                                    closable
                                    class="drag-icon"
                                    @close="handleButtonsClose(index)"
                                    @click="openButtonsAddUpdate(tag, index)"
                                >
                                    {{ tag.name }}
                                </el-tag>
                            </Draggable>
                            <div>
                                <el-button
                                    ...
                                    @click="openButtonsAdd"
                                >
                                    额外操作按钮
                                </el-button>
                            </div>
                        </el-form-item>
                        <el-form-item class="item-area" label="表单项">
                            <div class="content">
                                <div class="like-table">
                                    <div class="l-head">
                                        <div style="width: 20%;">
                                            ...
                                            <el-tooltip
                                                class="item"
                                                effect="dark"
                                                content="格式：({table}){column}，“table”必需与主键的“table”一致，如果是附件则为附件的type"
                                            >
                                                <i class="iconfont icon-question-circle"></i>
                                            </el-tooltip>
                                        </div>
                                        ...
                                        <div style="width: 10%;">
                                            ...
                                            <el-tooltip
                                                class="item"
                                                effect="dark"
                                                content="尽量把不显示字段放到最后，容易判断页面上字段的排版"
                                            >
                                                <i class="iconfont icon-question-circle"></i>
                                            </el-tooltip>
                                        </div>
                                        ...
                                        <div style="width: 10%;">
                                            ...
                                            <el-tooltip
                                                class="item"
                                                effect="dark"
                                                content="如一行排放的元素不满每行记录数，则元素长度为：合并长度/每行记录数"
                                            >
                                                <i class="iconfont icon-question-circle"></i>
                                            </el-tooltip>
                                        </div>
                                        ...
                                        <div style="width: 10%;">
                                            ...
                                            <el-tooltip
                                                class="item"
                                                effect="dark"
                                                content="因为中文占3个字符，所以需要输入中文的字符长度，是数据库中的1/3"
                                            >
                                                <i class="iconfont icon-question-circle"></i>
                                            </el-tooltip>
                                        </div>
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            <div style="width: 10%;">
                                                <span>{{ el.show | filterBoolean }}</span>
                                            </div>
                                            <div style="width: 10%;">
                                                <div>
                                                    {{ el.type | filterType }}
                                            ...
                                            <div style="width: 5%;">
                                                <span>{{ el.nullable | filterBoolean }}</span>
                                            </div>
                                            <div style="width: 10%;">
                                                <span>{{ el.editable | filterBoolean }}</span>
                                            </div>
                                            <div style="width: 10%;">
                                                <span>{{ el.length || '-' }}</span>
                                            </div>
                                            <div style="width: 10%;">
                                                <span>{{ filterValidation(el.validationRuleId) }}</span>
                                            </div>
                                            <div style="width: 10%;">
                                                <span>{{ el.jsValidator }}</span>
                                            </div>
                                            <div
                                                style="width: 30px;text-align: center;line-height: 33px;"
                                            >
                                                <i
        ...
        <FormProp
            ...
            :default-list="column"
            :validation-rule-id-options="validationRuleIdOptions"
            @ok="onColumnOk"
        />
        <!--平铺表单建模修改-->
        <FormBuildEdit
...
import ExtraOperationBtns from './components/ExtraOperationBtns' // 额外操作按钮弹框
import {
    Api_FormModelBuild,
...
import img_subTitle from '@/assets/images/tips/detail-sub-title.png'
...
export default {
    ...
    methods: {
        ...
        handleSure () {
            ...
            const { column: columns, buttons } = this
        ...
        // 打开新增列模态框
        openColumnAdd () {
            this.parentPrimaryKey = this.addFormData.primaryKey.split(')')[0] + ')'
        ...
        // 打开新增额外操作按钮模态框
        openButtonsAdd () {
            this.$refs.ref_buttons.openModel()
        },
        // 打开修改额外操作按钮模态框
        openButtonsAddUpdate (row, updateIndex) {
            this.$refs.ref_buttons.openModel(row, updateIndex)
        },
        // 新增额外操作按钮成功
        onButtonsOk (data, updateIndex) {
            if (updateIndex !== undefined) {
                this.buttons.splice(updateIndex, 1, { ...data })
            } else {
                this.buttons.push({ ...data })
            }
        },
        // 删除额外操作按钮时
        handleButtonsClose (index) {
