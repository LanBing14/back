<template>
    <div class="list-wrap">
        ...
        <div v-loading="listLoading">
            ...
            <p v-if="filterData.length===0" style="text-align: center;padding: 40px 0;">{{ emptyText }}</p>
        </div>
        <a-pagination
            v-if="total>0"
            v-model="currentPage"
    ...
    export default {
        ...
        computed: {
            filterData() { // 过滤生成后的数据
                ...
                this.list.forEach(item => {
                    const rowDate = new Date(item[timeKey]);
                ...
                return a.map(item => {
                    if (item) {
                        if (preItem) {
                            lastIndex += preItem.data.length;
                        }
                        item.lastIndex = lastIndex;
                        preItem = Object.assign({}, item)
                    }
                    return item
                })
            }
        ...
        methods: {
            ...
            tableRowClassName({row}) {
                return this.rowClassName(row) ? 'no-read' : '';
            }
        }
    }
</script>
