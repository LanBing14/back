<template>
  <div class="inputSearch-wrap" @click.stop>
    <a-input
      ...
      @input="onInput()"
    />
    <div v-loading="loading" v-if="resultShow" class="search-result-wrap">
      <vue-scroll>
        <ul class="search-ul">
          <li
            v-for="(el, index) in result"
            :key="index"
            :title="el.name+'（'+el.username+'）'"
            @click="handleChange(el)"
          >
            <span>{{ el.name + (el.username ? ('（'+el.username+'）') : '') }}</span>
          </li>
...
export default {
  ...
  data() {
    return {
      ...
      loading: false // 搜索时的loading状态
    };
  },
  watch: {
    defaultData(newDataArr) {
      newDataArr.forEach(oldItem => {
        const findRes = this.checkedList.find(
      ...
      //     const findRes = newDataArr.find(newItem => oldItem.id === newItem.id);
      //     console.log(newDataArr,findRes)
      //     if (!findRes) {
      //         this.handleChange(oldItem)
      //     }
      // })
  ...
  methods: {
    ...
    onInput() {
      ...
      if (this.inputVal) {
        ...
        this.inputTimer = setTimeout(() => {
          this.checkedList = [];
          this.resultShow = true;
          this.fetchDataFn(this.inputVal)
            .then(res => {
              this.result = res;
              this.$emit("update:data", res);
              const defaultDataArr = this.defaultData;
              this.result.forEach(newItem => {
                const findRes = defaultDataArr.find(
                  defaultItem => newItem.id === defaultItem.id
                );
                if (findRes) {
    ...
    filterSelected(value) {
      const res = this.checkedList.find(item => item.id === value);
      if (res) {
        return true;
      } else {
        return false;
